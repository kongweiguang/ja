...
def run_java(property_path: Path, digest: str, classification_digest: str, temp: Path) -> None:
    ...
    require_marker("java-build", build_result.stdout + build_result.stderr, r"Tests run:\s*248, Failures:\s*0, Errors:\s*0")
...
def run_typescript(property_path: Path, digest: str, classification_digest: str, temp: Path) -> None:
    ...
    require_marker("typescript-suite", suite.stdout + suite.stderr, r"Tests\s+224 passed\s+\(224\)")
    print("TS_SUITE_OK tests=224")
    ts_filters = (
    ...
    for filter_id, selector in ts_filters:
        ...
        require_marker(
            f"typescript-filter-{filter_id}",
            filtered.stdout + filtered.stderr,
            r"Tests\s+1 passed(?:\s+\|\s+[0-9]+\s+skipped)?\s+\(224\)",
        )
