...
def repo_artifact_snapshot() -> dict[str, tuple[str, int, int]]:
    """Recursively fingerprint bounded build/cache roots so additions and rewrites cannot hide."""
    ...
    while pending:
        ...
        found[relative] = ("dir" if path.is_dir() else "file", stat.st_size, stat.st_mtime_ns)
        ...
        for path in children:
            if path.is_dir():
                ...
                if path.name == "__pycache__":
                    relative = path.relative_to(ROOT).as_posix()
                    stat = path.stat()
                    found[relative] = ("dir", stat.st_size, stat.st_mtime_ns)
                    pending.append(path)
                else:
                    pending.append(path)
                continue
...
def assert_no_repo_artifacts(before: dict[str, tuple[str, int, int]]) -> None:
    """Fail closed on new or changed bytecode/cache/build entries while preserving user targets."""
    after = repo_artifact_snapshot()
    changed = sorted(relative for relative, stamp in after.items() if before.get(relative) != stamp)
    removed = sorted(relative for relative in before if relative not in after)
    differences = changed + removed
    if differences:
        fingerprint = hashlib.sha256("\0".join(differences).encode("utf-8")).hexdigest()[:16]
        raise GateFailure(f"FAIL stage=cleanup classification=repo_artifact_created outputHash={fingerprint}")
...
def run_java(property_path: Path, digest: str, classification_digest: str, temp: Path) -> None:
    ...
    require_marker("java-build", build_result.stdout + build_result.stderr, r"Tests run:\s*64, Failures:\s*0, Errors:\s*0")
