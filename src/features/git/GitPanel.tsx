 ...
 */
export function GitPanel({ branch = "未选择分支", ahead = 0, behind = 0, files = [], commits = [], diffStat, loading = false, error, onRetry }: GitPanelProps): ReactElement {
  ...
  return (
    <div className="ja-git-panel">
      <div className="ja-git-summary">
        <div className="ja-git-branch"><GitBranch aria-hidden="true" /><strong>{branch}</strong></div>
        <span className="ja-git-sync">领先 {ahead} · 落后 {behind}</span>
      </div>
      <section className="ja-git-section" aria-labelledby="ja-git-files-title">
        ...
        {files.length === 0 ? <p className="ja-git-muted">没有未提交的文件变化。</p> : <ul className="ja-git-file-list">{files.map((file) => <li key={`${file.status}:${file.path}`}><span className={`ja-git-status ja-git-status-${file.status}`} aria-label={statusLabel(file.status)}>{statusLetter(file.status)}</span><span className="ja-git-file-path" title={file.path}>{file.path}</span><span className="ja-git-file-stat">{formatStat(file)}</span></li>)}</ul>}
      </section>
